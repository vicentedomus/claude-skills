...
<!-- BEGIN repo-local: claude-skills (no upstream) — reaplicar tras cada `npx skills add teng-lin/notebooklm-py` -->
## ⚡ En este repo (claude-skills) — instalación y auth

> Sección **local** de `claude-skills` (NO viene de upstream). El resto del archivo es el SKILL.md original de [`teng-lin/notebooklm-py`](https://github.com/teng-lin/notebooklm-py), vendorizado y pineado vía `skills-lock.json`. **Para actualizar:** re-corre `npx skills add teng-lin/notebooklm-py` y vuelve a aplicar este bloque `repo-local`.

**Instalación (contenedor efímero → bootstrap por sesión):**
```bash
uv tool install notebooklm-py        # sin [browser] basta para consultar/generar/descargar
export PATH="$HOME/.local/bin:$PATH"
notebooklm list                      # verifica auth (ver abajo)
```
El extra `[browser]`/Playwright **solo** hace falta para el `notebooklm login` interactivo, que se corre en un **desktop con pantalla**, no aquí.

**Auth — desktop (ruta primaria, la más simple):**
```bash
notebooklm login        # abre navegador → login Google → guarda storage_state.json
notebooklm list         # Auth ✓
```

**Auth — Claude Code on the web (headless — probado y funcionando):**
El entorno web es **efímero** y su config solo ofrece un `.env` **en texto plano visible** (sin almacén de secrets). El blob `storage_state.json` es una **credencial portadora** (cookies vivas de tu Google) → úsalo solo si aceptas ese riesgo en TU environment privado.

1. En tu **desktop**, tras `notebooklm login`, codifica el blob a base64 de **una línea** (evita el multi-línea/CRLF que rompe el `.env`):
   ```powershell
   # Windows PowerShell
   [Convert]::ToBase64String([IO.File]::ReadAllBytes("$HOME\.notebooklm\profiles\default\storage_state.json")) | Set-Clipboard
   ```
   ```bash
   # macOS / Linux
   base64 -w0 ~/.notebooklm/profiles/default/storage_state.json    # copia la salida
   ```
2. Pégalo en el `.env` del environment como `NOTEBOOKLM_AUTH_B64=<base64>` (una sola línea). Abre **sesión nueva** (los env vars aplican a sesiones nuevas).
3. Decodifica al vuelo y usa (nunca imprimas el valor):
   ```bash
   export NOTEBOOKLM_AUTH_JSON="$(printf '%s' "$NOTEBOOKLM_AUTH_B64" | base64 -d)"
   notebooklm list        # ✅ RPC autenticado a través del proxy de Claude Code on the web
   ```

**Gotchas verificados:**
- **`notebooklm doctor` da falso negativo con auth por env-var** (muestra `Auth ✗ not authenticated`): valida el **perfil en disco**, no `NOTEBOOKLM_AUTH_JSON`. **Verifica con `notebooklm list`** (o `notebooklm auth check --test --json`) — hacen una llamada real.
- Egress a `notebooklm.google.com` / `accounts.google.com` **sí pasa** por el proxy de Claude Code on the web.
- **Nunca** comitees el blob, **nunca** lo imprimas, y `unset NOTEBOOKLM_AUTH_JSON` al terminar. Un `notebooklm login` fresco **rota** las cookies.
<!-- END repo-local -->
