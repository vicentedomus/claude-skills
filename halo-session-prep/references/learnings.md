...
- **Formato real de los catálogos (¡las queries de ejemplo fallaban!):** `monstruos.cr` es un STRING con XP embebido (`"2 (XP 450; PB +2)"`) — filtrar con `cr LIKE '2 (%'`, NUNCA `cr::text = ANY(...)` (devuelve vacío). `entorno` = listas en inglés (Urban, Forest…). Los statblocks NPC (Bandit, Tough, Bandit Captain, Mage, Gladiator…) salen por `tipo ILIKE '%Humanoid%'`. **Tesoros se buscan en `items_catalog`** (no `items`); su tier `uncommon` (nivel ~4) son casi todos armas/armaduras +1 — los Wondrous vistosos están como `rare`.
