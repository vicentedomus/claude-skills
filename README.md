...
| [notebooklm](notebooklm/) | API completa de Google NotebookLM (crear notebooks, agregar fuentes, generar podcasts/videos/quizzes/mapas mentales y descargar en varios formatos, incluidas cosas fuera del web UI) — **vendorizada de [`teng-lin/notebooklm-py`](https://github.com/teng-lin/notebooklm-py) vía `npx skills add` (pineada en `skills-lock.json`) con una sección `repo-local` propia**; envuelve el paquete PyPI `notebooklm-py` (auto-install `uv`/`pip`, necesita egress a Google). Auth: `notebooklm login` (desktop, primaria) o `NOTEBOOKLM_AUTH_JSON`/base64 en `.env` (Claude Code web headless, probado). Se activa con `/notebooklm` |
| [oneshot](oneshot/) | Arma una one-shot de D&D lista para dirigir en QuestKeep por dos carriles —adaptar una aventura publicada (`data/books/`) o inventar una original con el compendio como musa y **reskin**— y la entrega como **plan modular** (árbol de bloques del Planeador, spec 007), no como página. Pipeline de nueve pasos con el reloj y el presupuesto como chequeos tempranos — **requiere un clon de `questkeep`** (manuales `docs/lazy-dm-manual.md` y `docs/flee-mortals-manual.md`, datos `data/5e/`, compendio) **+ el MCP de Supabase**; el carril B se apoya en `dnd-worldbuilder` para nombres y mundo |
| [patch-notes-animadas](patch-notes-animadas/) | Convierte PRs mergeados en "patch notes" animadas para usuarios no técnicos: MP4 corto (WhatsApp) y/o artifact con URL permanente, siempre sobre **capturas de la UI real** tomadas con Playwright (nunca UI redibujada). Repo-agnóstica: detecta la paleta del `:root` del proyecto y descubre cómo arrancarlo (harness E2E → dev server → deploy); pregunta **móvil o desktop**, que cambia viewport, formato y guion — **requiere la skill `huashu-design` al lado** (aporta la gramática de movimiento y sus pitfalls). Instala `ffmpeg-static` si el entorno no trae ffmpeg |
